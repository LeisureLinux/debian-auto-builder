...
func handleScan(w http.ResponseWriter, r *http.Request) {
	...
	for i, pkg := range trackedPackages {
		...
		if !result.InDebian && (result.Amd64Available || result.Arm64Available) {
			fmt.Printf("✅ Package %s: GitHub has builds, apt-repo should build from source\n", pkg.Package)
			triggerBuildForPackage(pkg) // 这里可以触发构建流程
		} else if !result.InDebian && (!result.Amd64Available || !result.Arm64Available) {
			fmt.Printf("⚠️ Package %s: GitHub releases missing architectures\n", pkg.Package)
			triggerBuildForPackage(pkg) // 这里也可以触发构建流程
...
func hasDebAsset(assets []Asset) bool {
	for _, asset := range assets {
		if strings.HasSuffix(asset.Name, ".deb") && 
		   (strings.Contains(asset.Name, "amd64") || 
			strings.Contains(asset.Name, "arm64")) {
			return true
		}
	}
	return false
